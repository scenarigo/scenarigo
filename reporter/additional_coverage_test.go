...
func TestWithNoColor(t *testing.T) {
	// Test WithNoColor function
	var contextNoColor bool
	...
	Run(func(r Reporter) {
		...
		contextNoColor = rep.context.noColor
	}, WithNoColor())

	if !contextNoColor {
		t.Error("WithNoColor should set noColor to true")
	}
}
...
func TestNewTestSummary(t *testing.T) {
	...
	summaryStr := summary.String(false) // Pass false for noColor parameter
	if summaryStr == "" {
...
func TestSkipColor(t *testing.T) {
	...
	rep.context = &testContext{noColor: false}
	...
	rep.context.noColor = true
	skipColorObj = rep.skipColor()
	if skipColorObj == nil {
		t.Error("skipColor should return a color object even with noColor")
	}
}
...
func TestPassColor(t *testing.T) {
	...
	passColorObj = rep.passColor()
	if passColorObj == nil {
		t.Error("passColor should return a color object even with noColor")
	}
}
...
func TestFailColor(t *testing.T) {
	...
	failColorObj = rep.failColor()
	if failColorObj == nil {
		t.Error("failColor should return a color object even with noColor")
	}
}
...
func TestTestSummaryColorMethods(t *testing.T) {
	...
	output1 := summary.String(false) // noColor = false
	if output1 == "" {
		t.Error("Summary should produce output")
	}

	output2 := summary.String(true) // noColor = true
	if output2 == "" {
		t.Error("Summary should produce output even with noColor")
	}
}
