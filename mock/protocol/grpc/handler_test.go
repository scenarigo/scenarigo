...
func TestStreamHandler_failure(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"bidi stream: assertion runs before non-OK status": {
			// Even when a non-OK status is configured, the request stream must be
			// received and asserted; a mismatching request fails the assertion
			// instead of silently passing with the configured status.
			mocks: []protocol.Mock{{
				Protocol: "grpc",
				Expect:   yamlutil.RawMessage("messages:\n  - messageId: '1'"),
				Response: yamlutil.RawMessage("status:\n  code: Internal"),
