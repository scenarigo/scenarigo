...
> **Note:** `expect.messages` follows the same semantics as other list assertions: the listed elements are matched as a prefix, so extra streamed messages are allowed. To also pin the number of received messages, combine `assert.and` with a template predicate on the actual list (`{{size($) == N}}`, or equivalently `{{assert.length(N)}}`); to match only containment, use a `contains` assertion:
>
> ```yaml
> expect:
>   messages:
...
Template expressions in request messages can reference the preceding request messages using `request.messages[N]`.

#### Bidirectional Streaming

For bidirectional streaming RPCs, use `messages` for both request and response. Template expressions in request messages can reference already-received responses using `response.messages[N]`, which blocks until the Nth response is available, and already-sent requests using `request.messages[N]`:
...
> **Note:** A request message that references a response which the server only produces after receiving that request creates a circular dependency. To avoid hanging forever, the wait for a referenced response is bounded by the step `timeout` if set, and otherwise by a default guard (30s), after which the step fails with an error indicating a possible deadlock. An interrupted wait is reported as an error even inside `??` or `defined()`; only a definitive absence (the stream ended before the N-th message arrived) is treated as undefined, so `response.messages[N] ?? fallback` never silently falls back because of a timeout.
...
A plugin module that imports `github.com/zoncoen/query-go` (or one of its extractor modules) has its imports rewritten to the `/v2` paths before the build: the v1 and v2 module paths coexist, so such a plugin builds without an error and the on-failure migration below never sees it — it would keep running on the compatibility shims instead, with a v1 `QueryOptions` refused at registration and v1-shaped extractors served by the legacy adapter. Vendor trees are never entered, files that resolve outside the plugin directory through a symbolic link are left alone, and test files keep their v1 imports — the build does not compile them, so the migration below could never fix their extractor calls, and the v1 `require` then stays in `go.mod` for them as a test-only dependency; otherwise it is removed, and the build brings the `/v2` modules in itself. Unlike the on-failure migration, this rewrite is kept even when the build fails. `--skip-migration` disables this rewrite too.
