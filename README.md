...
A plugin module that imports `github.com/zoncoen/query-go` (or one of its extractor modules) has its imports rewritten to the `/v2` paths before the build: the v1 and v2 module paths coexist, so such a plugin builds without an error while its query options are silently dropped, and the on-failure migration below would never see it. Vendor trees are never entered, and test files keep their v1 imports — the build does not compile them, so the migration below could never fix their extractor calls, and the v1 `require` then stays in `go.mod` for them as a test-only dependency; otherwise it is removed, and the build brings the `/v2` modules in itself. Unlike the on-failure migration, this rewrite is kept even when the build fails. `--skip-migration` disables this rewrite too.
