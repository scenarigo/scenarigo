...
> **Note:** Unlike list assertions elsewhere (which match the listed elements as a prefix), `expect.messages` asserts the streamed response **exactly**: the number of received messages must equal the number listed, so a server that emits more messages than expected fails the assertion. To match only containment without the count, write `messages` as a `contains` assertion instead of a list:
>
> ```yaml
> expect:
>   messages:
...
> **Note:** A request message that references a response which the server only produces after receiving that request creates a circular dependency. To avoid hanging forever, the wait for a referenced response is bounded by the step `timeout` if set, and otherwise by a default guard (30s), after which the step fails with an error indicating a possible deadlock.
