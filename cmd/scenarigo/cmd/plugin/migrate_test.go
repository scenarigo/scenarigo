...
`},
		...
		"double negation stays an expression": {
			files: map[string]string{"main.go": "package main\n\n" + pluginImport + `
func F(ctx *plugin.Context) bool {
	_, ok := ctx.Vars().ExtractByKey("k")
	return !!ok
...
`},
		...
		"raw-string context import is reused": {
			files:         map[string]string{"main.go": "package main\n\nimport (\n\tgocontext `context`\n\n\tscenarigo \"github.com/scenarigo/scenarigo/context\"\n)\n\nvar _ gocontext.Context\n\nfunc F(vars scenarigo.Vars) bool {\n\t_, ok := vars.ExtractByKey(\"k\")\n\treturn ok\n}\n"},
			expectFiles:   map[string]string{"main.go": "package main\n\nimport (\n\tgocontext `context`\n\n\tscenarigo \"github.com/scenarigo/scenarigo/context\"\n)\n\nvar _ gocontext.Context\n\nfunc F(vars scenarigo.Vars) bool {\n\t_, err := vars.ExtractByKey(gocontext.Background(), \"k\")\n\treturn err == nil\n}\n"},
			expectChanges: []string{"gocontext.Background()"},
		},
...
func TestFileEdits_ApplyRefusesAChangedFile(t *testing.T) {
	// The edits were planned against a snapshot; a file that no longer
	// matches it was edited after the analysis, and writing would replace
	// that edit with a rewrite of the older text.
	dir := t.TempDir()
	path := filepath.Join(dir, "main.go")
	planned := "package main\n\nvar ok = true\n"
	edited := "package main\n\nvar ok = false\n"
	create(t, path, edited)
	off := strings.Index(planned, "ok")
	fe := &fileEdits{
		path: path, rel: "main.go", src: []byte(planned), mode: 0o644,
		edits: []edit{{start: off, end: off + 2, text: "err"}},
	}
	res := &migrationResult{originals: map[string]migratedSource{}}
	err := fe.apply(res, nil)
	if err == nil {
		t.Fatal("no error")
	}
	if want := "changed while it was being migrated"; !strings.Contains(err.Error(), want) {
	...
	if len(res.originals) != 0 {
		t.Errorf("an original was recorded for a file that was not written")
	}
}

func TestMigration_RecordsTheOriginalBeforeWriting(t *testing.T) {
	if testing.Short() {
		t.Skip("type-checks a module against the scenarigo checkout")
	}
	// The restore guard owns what record writes to; recording after the
	// write would leave a panic between the two with a rewritten file the
	...
	_, err := migrateExtractorCalls(t.Context(), &migration{
		...
		record: func(path string, s migratedSource) {
			...
			if !bytes.Equal(cur, s.content) {
				t.Error("record ran after the file was already rewritten")
			}
		},
	})
	if err != nil {
	...
	if recorded != 1 {
		t.Errorf("expected 1 recorded original but got %d", recorded)
	}
}

func TestMigration_LoaderUsesTheSelectedGoCommand(t *testing.T) {
	if testing.Short() {
		t.Skip("type-checks a module against the scenarigo checkout")
	}
	...
	// packages.Load always runs a command named "go". When SCENARIGO_GO
	// points at something not named that - go1.26, a wrapper - the loader
	// must still run it, or the build and the type-check use different
	// toolchains. The wrapper leaves a marker only for the loader's
	// -json list calls, which buildDeps never makes.
	realGo, err := exec.LookPath("go")
	if err != nil {
		t.Fatal(err)
	}
	tmp := t.TempDir()
	marker := filepath.Join(tmp, "loader-used-me")
	wrapper := filepath.Join(tmp, "go-custom")
	create(t, wrapper, "#!/bin/sh\ncase \"$*\" in *-json*) : >\""+marker+"\";; esac\nexec \""+realGo+"\" \"$@\"\n")
	...
	if len(res.changes) != 1 {
		t.Fatalf("expected 1 change but got %d: %q", len(res.changes), res.changes)
	}
	if _, err := os.Stat(marker); err != nil {
		t.Error("the loader did not run the selected go command")
	}
}
