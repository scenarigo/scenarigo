...
type migration struct {
	...
	target string
	// record, when set, is told about every original just before its file is
	// rewritten. The caller's restore guard owns what record writes to, so a
	// panic that never lets migrateExtractorCalls return still leaves the
	// guard holding every file already touched.
	record func(path string, s migratedSource)
	// goShimDir is a directory whose "go" runs the selected go command. It is
	// put first on the process PATH while packages.Load runs, whenever a go
	// command other than plain "go" is selected; a shim link is created only
	// when that command's own name is not "go".
	goShimDir string
}

// loaderPathDir returns the directory to put first on PATH so that a command
// named "go" runs the selected go command, and a cleanup. packages.Load
// resolves its go binary with exec.Command("go", ...), which searches the
// PATH of this process - not the PATH in its Env - so pointing the loader at
// the build's toolchain means changing the process PATH around the load. The
// empty string means plain "go" is already the selected command and nothing
// needs to change. Best effort: when the shim cannot be made, the loader
// falls back to whatever "go" resolves to, the pre-existing behaviour.
func loaderPathDir(goCmd string) (string, func()) {
	none := func() {}
	if goCmd == "" || goCmd == "go" {
		return "", none
	}
	abs := goCmd
	if resolved, err := exec.LookPath(goCmd); err == nil {
		abs = resolved
	}
	if a, err := filepath.Abs(abs); err == nil {
		abs = a
	}
	if filepath.Base(abs) == "go" {
		return filepath.Dir(abs), none
	}
	dir, err := os.MkdirTemp("", "scenarigo-go-shim-")
	if err != nil {
		return "", none
	}
	if err := os.Symlink(abs, filepath.Join(dir, "go")); err != nil {
		_ = os.RemoveAll(dir)
		return "", none
	}
...
func migrateExtractorCalls(ctx context.Context, m *migration) (*migrationResult, error) {
	...
	if dir, cleanup := loaderPathDir(m.goCommand()); dir != "" {
		m.goShimDir = dir
		defer cleanup()
	}
	...
	for _, fe := range edits {
		if err := fe.apply(res, m.record); err != nil {
			return res, err
		}
	}
	return res, nil
...
func loadPluginPackages(ctx context.Context, m *migration, dir, pattern string) ([]*packages.Package, error) {
	...
	// it with the import paths of the dependencies. file= loads the whole
	// package containing the file, so the type information and the name-
	// freedom checks see sibling files a single-file build does not compile.
	// That only errs toward caution - a name a sibling occupies is treated
	// as taken and the call goes to a manual warning instead of a rewrite -
	// and the edits themselves are limited to the target file afterwards.
	root := pattern
	if strings.HasSuffix(root, ".go") {
		root = "file=" + root
	}
	// packages.Load resolves its go binary with the PATH of this process, not
	// the one in cfg.Env, so the selected toolchain has to sit there for the
	// duration of the load.
	if m.goShimDir != "" {
		orig, had := os.LookupEnv("PATH")
		if err := os.Setenv("PATH", m.goShimDir+string(os.PathListSeparator)+orig); err == nil {
			defer func() {
				if had {
...
// loadEnv returns the environment for packages.Load, with the directory of
// the selected go command first on its PATH. Note that the loader resolves
// its go binary with the PATH of this process, not this one - see the swap
// around packages.Load - but the subprocesses of the loaded go command see
// this PATH.
...
func (fe *fileEdits) apply(res *migrationResult, record func(string, migratedSource)) error {
	...
	// The edits were planned against fe.src; a file that no longer matches
	// it was edited after the analysis, and writing would overwrite that
	// edit with a rewrite of the older text.
	if cur, err := os.ReadFile(fe.path); err != nil {
		return fmt.Errorf("failed to reread %s: %w", fe.rel, err)
	} else if !bytes.Equal(cur, fe.src) {
		return fmt.Errorf("%s: the source changed while it was being migrated", fe.rel)
	}
	...
	if record != nil {
		record(fe.path, migratedSource{content: fe.src, mode: fe.mode})
	}
...
func (p *planner) conversionOf(def *ast.Ident, obj *types.Var, uses []*ast.Ident) *conversion {
	...
	for _, id := range uses {
		...
		case *ast.BinaryExpr, *ast.UnaryExpr:
			// A bare comparison would bind wrongly there: !ok under another
			// unary NOT would become !err != nil, which does not compile.
			text = "(" + text + ")"
		}
...
// importPath returns the import path of spec, whether it is written with
// double quotes or as a raw string literal.
...
func (p *planner) stdContextName() (string, *edit) {
	for _, imp := range p.file.Imports {
		if importPath(imp) != stdContextPath {
			continue
		}
		if imp.Name == nil {
...
func (p *planner) nameIsFree(name string) bool {
	for _, imp := range p.file.Imports {
		...
		if path.Base(importPath(imp)) == name {
			return false
		}
	}
