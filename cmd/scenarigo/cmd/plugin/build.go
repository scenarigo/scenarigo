...
// rewrite may touch. A file that resolves outside dir through a symbolic link
// is left alone for the same reason. Test files are skipped when
// skipTestFiles says so - the build does not compile them, so a rewrite there
// is not needed to build and cannot be followed up by the call migration,
// which only sees what a failed build compiles.
//
// Every file is parsed and formatted before anything is written, so a source
// the parser rejects fails the rewrite with no file touched; only a write
// error can leave it partially applied.
func rewriteImports(dir string, skipTestFiles bool, rewrite func(string) string) error {
	...
	if err := filepath.Walk(dir, func(path string, info fs.FileInfo, err error) error {
		...
		if !within(realPath(path), realDir) {
			return nil
		}
		...
		r := &rewritten{path: path, mode: info.Mode()}
		if err := format.Node(&r.out, fset, f); err != nil {
	...
	for _, r := range outs {
		if err := os.WriteFile(r.path, r.out.Bytes(), r.mode); err != nil {
			return fmt.Errorf("failed to modify import path: %w", err)
		}
	}
	return nil
...
func createPluginBuilder(cmd *cobra.Command, goCmd string, pluginModules map[string]*overrideModule, root, pluginDir string, item schema.OrderedMapItem[string, schema.PluginConfig], opts *buildOpts) (*pluginBuilder, func(), error) {
	...
	if !opts.skipMigration {
		...
		// runs on a build failure never sees it. What it built would then
		// live on the compatibility shims: a protocol's v1 QueryOptions is
		// refused at registration and v1-shaped extractors only work through
		// the legacy adapter. Rewrite the imports up front instead. A compile
		// error that follows, such as an extractor call with the v1
		// signature, is exactly what the on-failure migration exists to fix.
