...
func init() {
	...
	runCmd.Flags().BoolVarP(&verbose, "workers", "w", false, "specify the number of workers to run tests in parallel (the default value is the number of logical CPUs usable by the current process)")
...
func run(cmd *cobra.Command, args []string) error {
	...
	if cfg != nil && cfg.Execution.Workers > 0 {
		parallelNum = cfg.Execution.Workers
	}
	if workers > 0 {
		parallelNum = workers
	}
