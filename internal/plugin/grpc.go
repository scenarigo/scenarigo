...
// validateStreamValue rejects nil (or typed-nil) streams returned together with
// a nil error by a custom client. Such a stream would pass the open step and
// then panic on the first reflective Send/Recv call with a nil receiver.
func validateStreamValue(stream reflect.Value) error {
	v := stream
...
// isNilable reports whether reflect.Value.IsNil is legal for the kind.
...
func GRPCStreamRequestType(method reflect.Value, methodType GRPCMethodType) (reflect.Type, error) {
	...
	case GRPCMethodClientStream, GRPCMethodBidiStream:
		...
		// Send method signature: func(*Req) error.
		// For concrete (non-interface) stream types, In(0) is the receiver.
		argIdx := 0
		if streamType.Kind() != reflect.Interface {
			argIdx = 1
		}
		if sendMethod.Type.NumIn() <= argIdx {
			return nil, errors.New("Send method has no message argument")
		}
		reqType := sendMethod.Type.In(argIdx)
		if reqType.Kind() != reflect.Ptr {
			return nil, errors.Errorf("Send method argument must be a pointer to a message but got %s", reqType.String())
		}
		return reqType.Elem(), nil
	default:
		// Unary and server streaming take the request as the second argument:
		// func(ctx, *Req, ...CallOption). Guard the reflection so a malformed
		// signature returns an error instead of panicking.
		if mt.NumIn() < 2 {
			return nil, errors.Errorf("method has no request argument (got %d)", mt.NumIn())
		}
		reqType := mt.In(1)
		if reqType.Kind() != reflect.Ptr {
			return nil, errors.Errorf("request argument must be a pointer to a message but got %s", reqType.String())
		}
		return reqType.Elem(), nil
	}
}
...
func GRPCStreamAsClientStream(stream reflect.Value) (grpc.ClientStream, bool) {
	// Guard IsNil with a kind check: calling IsNil on a non-nilable kind
	// (e.g. a concrete struct stream value) panics.
	if !stream.IsValid() || (isNilable(stream.Kind()) && stream.IsNil()) {
		return nil, false
	}
