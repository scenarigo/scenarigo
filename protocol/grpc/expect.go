...
func (e *Expect) Build(ctx *context.Context) (assert.Assertion, error) {
	...
	// Streaming responses are asserted exactly: the message count must match, not
	// just the listed prefix.
	msgsAssertion, err := assert.Build(ctx.RequestContext(), e.Messages, assert.FromTemplate(ctx), assert.ExactList())
	if err != nil {
